...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt:
                label_file = labels_dir / (path.stem + '.txt')  # 对应输出文件路径

                # 获取索引 i，如果找不到文件，则返回 -1
                try:
                    i = labels_list.index(str(path.stem) + '.txt')
                except ValueError:
                    # 文件名不在列表中，可以设置 i = -1 或者其他标记
                    i = -1
                    # 可选：打印警告
                    print(f"Warning: {path.stem}.txt not in labels_list, using i=-1")

                gn = torch.tensor(shapes[si][0])[[1, 0, 1, 0]]  # normalization gain whwh

                for *xyxy, conf, cls in predn.tolist():
                    xywh = (xyxy2xywh2(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # normalized xywh
                    # 如果 save_conf=True，则包含置信度
                    line = ((i+1) if i >= 0 else 0, *xywh, conf) if save_conf else ((i+1) if i >= 0 else 0, *xywh)
                    # 将预测写入对应 label 文件
...
if __name__ == '__main__':
    ...
    p = "/mnt/data1/hty/Documents/datasets/VEDAI_yolov5/fold01/labels/test"
