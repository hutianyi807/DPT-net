...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        z = []  # inference output 推理时保持所有层的检测结果
        logits_ = [] # 保存类别预测的原始 logits（未经过 sigmoid ）
...
class Model(nn.Module):
    ...
    def forward(self, x, x2, augment=False, profile=False): # [8, 3, 640, 640]
        if augment: # 是否启用 test-time augmentation（多尺度 + 翻转）
            img_size = x.shape[-2:]  # height, width
            s = [1, 0.83, 0.67]  # scales 多尺度缩放因子
            f = [None, 3, None]  # flips (2-ud, 3-lr) 翻转：2 上下翻，3 左右翻
            y = []  # outputs
    ...
    def forward_once(self, x, x2, profile=False): # 按照 self.model 的模块列表执行
        y, dt = [], []  # outputs   y = [] 用来保存每一层的输出（按 index 存）
        i = 0
        for m in self.model: # len(self.model): 38
        ...
        if profile: # 是否启用测时/测 FLOPS（用于性能分析）
