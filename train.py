# CUDA_VISIBLE_DEVICES=2 nohup python train.py --data /mnt/data1/hty/Documents/algorithms/ICAFusion/data/multispectral/VEDAI.yaml > training_0.log 2>&1 &
import argparse
import logging
import math
import os
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            labels = np.array(labels, dtype=np.float32).reshape(-1, 5)  # 5 = [cls, x, y, w, h]
            ...
            if labels.shape[0] == 0:
                ...
                c = torch.tensor(labels[:, 0].astype(np.int64), dtype=torch.long)   # classes
                b = torch.tensor(labels[:, 1:], dtype=torch.float32) 
